...
contains
    ...
    real(DP) function rbeta(alpha, beta, seed) result(x)
        ...
        c = rgamma(alpha, 1.0_dp, seed)
        d = rgamma(beta, 1.0_dp, seed)
        x = c / (c + d)
    end function rbeta
    ...
    real(DP) function rgamma(alpha, beta, seed) result(x)
        ...
        if (alpha .gt. 1.0) then
            ...
            x = ms * (runif(0.0_dp, 1.0_dp, seed)**(1.0_dp / alpha)) / beta
        end if
    end function rgamma
    ...
    real(DP) function rlognormal(mu, sigma, seed) result(x)
        ...
        if (sigma .le. 0) then
            stop "Sigma^2 must be strictly positive."
        end if
    ...
    real(DP) function rt(v, seed) result(x)
        ...
        if (v .le. 0) then
            stop "Degress of freedom must be strictly positive."
        end if
